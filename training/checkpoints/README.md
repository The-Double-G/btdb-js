...
Each training generation starts from one immutable Hosted Model snapshot artifact. Distributed selection replaces the deterministic score-weighted aggregate of validated schema-13 policy bundles, the accumulated candidate-side strategy outcome records, generation metadata, and bounded two-policy population history in that snapshot; other shard-learned stores are not promoted. The 31,048-parameter bundle contains 5,707 strategy parameters and 25,341 parameters for the semantic-intent-spatial recurrent actor, value, and survival heads. This audit checkpoint can therefore contain the public aggregate model fields present in the hosted GET response.
